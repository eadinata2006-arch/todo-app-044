...
type TaskDetailCardProps = { todo: Todo };
    ...
    <main className="min-h-screen p-8 bg-gray-100">
      <div className="max-w-2xl mx-auto bg-white p-8 rounded-xl shadow-lg border border-gray-100">
        <header className="mb-6 border-b pb-4 flex items-center justify-between">
          ...
          <Link href="/" className="text-sm bg-gray-200 hover:bg-gray-300 text-gray-700 px-3 py-1.5 rounded-md transition">
            Kembali ke Daftar
          </Link>
        </header>
        <div className="space-y-4">
          ...
          <div>
            ...
            <p className="text-gray-600 bg-gray-50 p-4 rounded-md border border-gray-200 mt-1">{todo.description}</p>
          </div>
          <div>
            <label className="text-xs font-semibold text-gray-400 uppercase tracking-wider">STATUS</label>
            <div className="mt-1">
              <span className={`inline-block px-3 py-1 text-sm font-semibold rounded-full ${todo.completed ? 'bg-green-100 text-green-700 border border-green-200' : 'bg-yellow-100 text-yellow-700 border border-yellow-200'}`}>
